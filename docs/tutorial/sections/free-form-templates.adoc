...
<3> The `$<OBJ_NAME>` built-in refers the value in `objName`.  In templates generated form matrices with patterns, use `$<BASE_OBJ_NAME>` to refer to the undecorated element from the list that generated `$<OBJ_NAME>`.
<4> The `template` helper attribute marks the start of the YAML defining any arbitrary YAML to be deployed to Kubernetes.
...
<5> Defining environment variables in a container is the same whether using helper or raw templates.
