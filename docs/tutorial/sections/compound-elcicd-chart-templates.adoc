...
****
* Defining compound el-CICD Chart templates.
****
...
. Create and save the following file, and name it `elcicd-demo-2.yaml`:
+
...
.. Compound templates are listed under the `templateNames` attribute, versus the `templateName` for a single attribute.
