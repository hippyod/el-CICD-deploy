{{- define "elcicd-kubernetes.importEnvs" }}
  {{- $ := index . 0 }}
...
{{- define "elcicd-kubernetes.createProjectedVolumesAndMountsByLabels" }}
  {{- $ := index . 0 }}
  {{- $podValues := index . 1 }}
  ...
  {{- if or $.Release.IsUpgrade $.Release.IsInstall }}  
    ...
    {{- range $volumeByLabels := $containerVals.projectedVolumeLabels }}
      ...
      {{- if $labeledResources }}
        {{- include "elcicd-kubernetes.createProjectedVolumesByLabels" (list $ $podValues $volumeByLabels $labeledResources) }}
...
{{- define "elcicd-kubernetes.createProjectedVolumesByLabels" }}
  {{- $ := index . 0 }}
  {{- $podValues := index . 1 }}
